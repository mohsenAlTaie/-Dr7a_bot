...
conn = sqlite3.connect("vip.db", check_same_thread=False) c = conn.cursor() c.execute(""" CREATE TABLE IF NOT EXISTS vip_users ( user_id INTEGER PRIMARY KEY, expiry_date TEXT ) """) conn.commit()

def add_vip(user_id: int, days: int): expiry = datetime.utcnow() + timedelta(days=days) c.execute("REPLACE INTO vip_users (user_id, expiry_date) VALUES (?, ?)", (user_id, expiry.isoformat())) conn.commit()

def remove_vip(user_id: int): c.execute("DELETE FROM vip_users WHERE user_id = ?", (user_id,)) conn.commit()

def is_vip(user_id: int): c.execute("SELECT expiry_date FROM vip_users WHERE user_id = ?", (user_id,)) row = c.fetchone() if row: expiry = datetime.fromisoformat(row[0]) return datetime.utcnow() < expiry return False

def get_vip_list(): c.execute("SELECT user_id, expiry_date FROM vip_users") return c.fetchall()
...
async def activate_vip(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query user_id = query.from_user.id add_vip(user_id, 30) await query.answer() await query.edit_message_text("✅ تم تفعيل اشتراك VIP لحسابك لمدة 30 يومًا!")
...
if user_data["count"] >= limit:
    await update.message.reply_text("🚫 وصلت للحد الأقصى من التحميلات اليومية.")
    return
...
async def addvip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE): try: user_id = int(context.args[0]) days = int(context.args[1]) add_vip(user_id, days) await update.message.reply_text(f"✅ تم إعطاء VIP للمستخدم {user_id} لمدة {days} يومًا.") except: await update.message.reply_text("❌ الاستخدام: /addvip [user_id] [days]")

async def removevip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE): try: user_id = int(context.args[0]) remove_vip(user_id) await update.message.reply_text(f"✅ تم إزالة VIP من المستخدم {user_id}.") except: await update.message.reply_text("❌ الاستخدام: /removevip [user_id]")

async def viplist_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE): users = get_vip_list() if not users: await update.message.reply_text("📭 لا يوجد مستخدمين VIP") else: text = "💎 قائمة VIP:\n" for user_id, expiry in users: text += f"- {user_id} (ينتهي في {expiry})\n" await update.message.reply_text(text)

def main(): app = Application.builder().token(TOKEN).build() app.add_handler(CommandHandler("start", start)) app.add_handler(CommandHandler("usage", usage)) app.add_handler(CommandHandler("addvip", addvip_cmd)) app.add_handler(CommandHandler("removevip", removevip_cmd)) app.add_handler(CommandHandler("viplist", viplist_cmd)) app.add_handler(CallbackQueryHandler(show_commands, pattern="commands")) app.add_handler(CallbackQueryHandler(show_vip_info, pattern="vip_info")) app.add_handler(CallbackQueryHandler(activate_vip, pattern="activate_vip")) app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_video)) app.run_polling()
